...
class GraghTransformer(nn.Module):
	...
	def __init__(
			...
			in_channels, 
			edge_features=10,
			num_hidden_channels=128, #  default = 128
            activ_fn=nn.SiLU(),
			transformer_residual=True,
			num_attention_heads=4,
			norm_to_apply='layer', #default = batch
			dropout_rate=0.1,
			num_layers=4,
			**kwargs
			):
		...
		self.output_linner = nn.Linear(num_hidden_channels,2)
	...
	def forward(self, node_s, edge_s, edge_index):		
		...
		#return node_feats
		node_feats = self.output_linner(node_feats)
		return node_feats
