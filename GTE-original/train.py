...
def train_one_epoch(model, data_loader):
    ...
    for data in data_loader:
        ...
        _, _, labels, node_features, G_batch, adj_matrix, xyz_feats, edges, edge_att, edge_feat = data
        ...
        y_pred = model(node_features, xyz_feats, edges, edge_att, edge_feat, adj_matrix)
...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            sequence_names, _, labels, node_features, G_batch, adj_matrix, xyz_feats, edges, edge_att, edge_feat = data
...
def train(model, train_dataframe, valid_dataframe, fold = 0):
    train_loader = DataLoader(dataset=ProDataset(train_dataframe), batch_size=BATCH_SIZE, shuffle=True, num_workers=8, collate_fn=graph_collate)
    valid_loader = DataLoader(dataset=ProDataset(valid_dataframe), batch_size=BATCH_SIZE, shuffle=True, num_workers=8, collate_fn=graph_collate)
...
def train_full_model(all_dataframe, aver_epoch):
    ...
    train_loader = DataLoader(dataset=ProDataset(all_dataframe), batch_size=BATCH_SIZE, shuffle=True, num_workers=8, collate_fn=graph_collate)
